-- 1) Causa raiz: função de recálculo sem GRANT de execução para o app.
GRANT EXECUTE ON FUNCTION public.recompute_card_invoice(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.recompute_card_invoice(uuid) TO service_role;

-- 2) Rotina de reconstrução em lote (workspace).
...
GRANT EXECUTE ON FUNCTION public.recompute_workspace_invoices(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.recompute_workspace_invoices(uuid) TO service_role;

-- 3) Health check de integridade financeira.
...
GRANT EXECUTE ON FUNCTION public.financial_health_check(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.financial_health_check(uuid) TO service_role;

-- 4) Reconstrução imediata de todas as faturas existentes.
DO $$
DECLARE r RECORD;
BEGIN
